#!/usr/bin/env python3
"""Deploy contracts/dkim_probe.py to GenLayer Testnet Bradbury (chain id 4221).

The contract is deployed as source; deploy gas is dominated by the calldata,
which is why the build refuses anything over 19 000 bytes. Run
...
import bradbury
from bradbury import MAX_TX_GAS, POLL_INTERVAL_MS, POLL_RETRIES
...
def main():
    ...
    if address:
        ...
        print("\nnext:")
        print("  python3 scripts/attest_bradbury.py %s <BLOB_URL>" % (address,))
