#!/usr/bin/env python3
"""Send one attest(blob_url) write to a deployed dkim probe on Bradbury.

The URL is the only thing that reaches calldata. The blob it points at is read
by the leader and by every validator inside the non-deterministic block, and
only the canonical string is agreed on and stored: domain, selector, bh, the
SHA-256 of the Message-ID, the key size, the verdict and a reason.
...
def main():
    ...
    encoded = bradbury.write_calldata(client, account, address, "attest", [blob_url])
    gas = bradbury.estimate(client, account, encoded, final=False)

    print("\nsubmitting attest transaction ...")
    started = time.time()
    ...
    if record is UNKNOWN:
        return
    print("\nrecord %d:" % (record_id,))
    for key in ("domain", "selector", "bh", "message_id_sha256", "key_bits",
                "valid", "reason"):
