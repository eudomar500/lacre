#!/usr/bin/env python3
"""Drive contracts/value_probe.py against a stubbed SDK.

The contract is executed as written, with stand-ins for the parts of the
runner it touches: storage, gl.message, the self balance and the outgoing
message. Nothing is deployed and no value moves.
...
posted with emit_transfer is queued rather than settled, because the consensus
contract executes it after the transaction finalizes. Whether it settles at
all, and whether an externally owned account can be the recipient, is what the
on-chain run measures; this only fixes the shape of the answer.
...
CONTRACT = HERE.parent / "contracts" / "value_probe.py"
    ...
    def contract_at(self, address):
        ...
        class Proxy:
            def emit_transfer(self, *, value, on="finalized"):
                ...
                # Queued, not settled: the consensus contract runs it later.
                node.transfers.append((address.as_hex, int(value), on))
...
def main():
    ...
    report.check("and queues nothing", node.transfers == [])
    ...
    report.check("still nothing queued", node.transfers == [])
    ...
    report.check("one transfer is queued to the wallet, on finalized",
                 node.transfers == [(WALLET, held, "finalized")],
                 str(node.transfers))
