...
MAX_BLOB = 16384
MAX_URL = 512
# Returned by get() with every record. Records of v1 and v1.1, which carry no
# such field, are schema 1.
SCHEMA_VERSION = "2"
# l= signs a prefix of the body, and a record cannot say which prefix.
...
def cached_key(router, domain, selector):
    # The KeyCache is looked up on every call, so it can be replaced without
    # a new Verifier. Nothing on this path may raise: a miss is a refusal.
...
class Contract(gl.Contract):
    ...
    record_count: u256
    # Per requester, keyed by as_hex: the return value of a write cannot be
    # read from the chain, so what each call did has to be a view.
    ...
    def _attest(self, url, blob, domain, selector):
        ...
        if parts[4] == NO_L:
            # Refused after the work, on the agreed verdict, so every
            # validator refunds alike.
